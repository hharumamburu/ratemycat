...
const getPost = async (id) => {
    try {
        const [rows] = await promisePool.execute('SELECT * FROM posts WHERE PostId = ?',
            [id]);
        console.log('rows', rows);
        return rows;
...
const addPost = async (params) => {
    try {
        const [rows] = await promisePool.execute(
            'INSERT INTO posts (Description, CreationDate, PicturePath) VALUES (?,?,?)',
            params);
        console.log('rows', rows);
        return rows;
...
const updatePost = async (params) => {
    try {
        const [rows] = await promisePool.execute(
            'UPDATE posts SET Description = ?, WHERE PostId = ?',
            params);
        console.log('rows', rows);
        return rows;
